...
## 5-1. VATSIM Connect (선택 기능, 승인 대기)

앱에 "Sign in with VATSIM" 버튼이 있지만 OAuth 클라이언트 승인 전에는 동작하지 않습니다.
설정 방법은 [vatsim-connect.md](vatsim-connect.md)를 보세요.

승인 전에 배포해도 심사에는 문제가 없습니다 (버튼을 누르면 "아직 설정되지 않았습니다"
안내가 뜨고, CID 직접 입력으로 모든 기능을 쓸 수 있습니다). 다만 심사자가 눌러 볼 수
있으므로, 승인 전에 올린다면 이 버튼을 숨기는 편이 깔끔합니다.
