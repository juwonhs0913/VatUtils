...
class VatFlightApp : Application() {
    ...
    override fun onCreate() {
        ...
        appScope.launch {
            ...
            runCatching {
                val tag = ServiceLocator.settingsRepository(this@VatFlightApp).current().languageTag
                if (tag.isNotBlank()) {
                    withContext(Dispatchers.Main) {
                        AppLanguage.apply(AppLanguage.fromTag(tag))
                    }
                }
            }.onFailure { Log.w("VATFlight", "언어 적용 실패", it) }

            // 첫 실행에서만 실제 적재가 일어나고, 이후에는 count() 한 번으로 끝납니다.
            runCatching {
                AirportSeeder.seedIfNeeded(this@VatFlightApp, AppDatabase.get(this@VatFlightApp))
            }.onFailure { Log.e("VATFlight", "공항 DB 시딩 실패", it) }

            // 설정이 켜져 있으면 감시 작업을 복구합니다 (기기 재부팅·앱 업데이트 후 대비).
            runCatching {
                val settings = ServiceLocator.settingsRepository(this@VatFlightApp).current()
                if (settings.notifyEnabled) {
                    ControllerWatchWorker.enable(this@VatFlightApp)
                }
            ...
            }.onFailure { Log.w("VATFlight", "관제소 감시 복구 실패", it) }
        }
    }
}
